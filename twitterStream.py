...
HTAG=["#testeteste"] # tag para configuracao
HTAG.append("#participabr") # tag básica
HTAG_=[HH.replace("#","NEW") for HH in HTAG]

from maccess import twss1 as tw
...
clients=[pymongo.MongoClient(uu) for uu in U]
#db = client[U.split("/")[-1]]
dbs = [client[uu.split("/")[-1]] for uu, client in zip(U,clients)]
#C = db[HTAG_] #collection
Cs = [db[HT] for db,HT in zip(dbs,HTAG_)]
